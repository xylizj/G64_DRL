...
void ConfigTxMsgA(uint8_t index, uint32_t id, uint8_t dl, uint32_t* data)
{
	...
	TxMsgBuffA[index].timer 	= SysTime;//Modified by xyl 2014.11.28
}
...
void ConfigRxMsgA(uint8_t index, uint32_t id, uint8_t dl, uint32_t* data)
{
	...
	RxMsgBuffA[index].timer 	=  SysTime;//Modified by xyl 2014.11.28
}
...
void Diag_InitTP(void)
{
	rx_state.timer = SysTime;//Modified by xyl 2014.11.28
	...
	tx_state.timer = SysTime;//Modified by xyl 2014.11.28
...
void MsgDIAG_RX_Decode(void)  
{
    RxMsgBuffA[MSG_DIAG_RX_INDEX].timer = SysTime;//ADDED by xyl 2014.11.28
	  RxMsgBuffA[MSG_DIAG_RX_INDEX].dl = CAN_DLC; //for DFEV company, dl must be 8, xyl 20151208 add
...
void Diag_TPTransmit(void)
{
	...
	else
	{
	    ...
	    //bs_sn = 1;
	    TxMsgBuffA[MSG_DIAG_TX_INDEX].state = PG_TX_REQ;
	    rx_state.timer = SysTime;//ADDED BY XYL 2015.4.15
...
void Diag_State(void)
{
	...
	{ 
		if(IO_ERR_OK == can_tx_msg(MSG_DIAG_TX))
		{
